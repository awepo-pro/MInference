    ...
    for tot in range(400_000, 500_000, 100_000):
        for pre in range(100_000, tot, 100_000):
            ...
            for _ in range(T + 1):
                ...
                if _:
                    minf_used += test_minf(pre, tot)

                gc.collect()
                torch.cuda.empty_cache()

                if _:
                    minf_prefix_used += test_minf_prefix_attention(pre, tot)

                gc.collect()
                torch.cuda.empty_cache()


            avg_minf_used = minf_used / T
            avg_minf_prefix_used = minf_prefix_used / T

            print(f'time: {avg_minf_used}')
